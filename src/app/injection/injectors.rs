...
use crate::app::engine::{efi_cfg::InjectorConfig, engine_status::InjectionInfo,sensors::SensorValues};

pub fn get_base_time(cfg: &InjectorConfig) -> f32 {
    return cfg.on_time - cfg.on_time;
}

pub fn get_battery_correction(base_time: &f32, cfg: &InjectorConfig, status: &SensorValues) -> f32 {
    let correction = cfg.battery_correction.map_or(1.0f32, |table|
        {
            let vbat_index = table.get(1).unwrap_or(&[0, 0]).into_iter().position(|vbat| *vbat <= (status.batt * 100.0) as i32);
...
pub fn get_pressure_correction() -> f32 {
    ...
    // FIXME: en teoría con llamar a set_injectorFlow ya estaría
    return 0.0;
}
...
pub fn fuel_mass_to_time(status: &InjectionInfo, fuel: f32) -> f32 {
    // TODO: revisar modo de inyección y cilindros
...
pub fn set_injector_flow(mut status: InjectionInfo, config: InjectorConfig) {
    // NOTE: como no se revisa la presión del combustible solo se llama una sola vez esto
